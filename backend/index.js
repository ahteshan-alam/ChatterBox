...
io.on("connection", (socket) => {
  socket.on("join-room", ({ id, formData }) => {
    ...
    rooms[formData.room] = rooms[formData.room].filter(
      (client) => client.id !== socket.id
    );
    rooms[formData.room].push({
      username: formData.username,
      id: socket.id,
      busy: false,
      partner: null,
  ...
  socket.on("typing", ({ username, room }) => {
    if (username === "") {
      ...
      socket
        .to(room)
        .volatile.emit("user-typing", { message: `${username} is typing ...` });
    }
  });
  socket.on("offer", (payload) => {
    ...
    const targetUser = rooms[socket.room]?.find(
      (client) => client.id === payload.target
    );
    ...
    if (targetUser && !targetUser.busy) {
      ...
      io.to(payload.caller.id).emit("userBusy", {
        message: `${payload.target} is busy in another call`,
      });
    }
  });
  socket.on("answer", (payload) => {
    ...
    rooms[socket.room].forEach((client) => {
      if (client.id === payload.caller.id || client.id === payload.target) {
        client.busy = true;
      }
    });
    const caller = rooms[socket.room]?.find(
      (client) => client.id === payload.caller.id
    );
    const callee = rooms[socket.room]?.find(
      (client) => client.id === payload.target
    );
    if (caller && callee) {
      rooms[socket.room].forEach((client) => {
        if (client.id === payload.caller.id) client.partner = callee.id;
      });
      rooms[socket.room].forEach((client) => {
        if (client.id === payload.target) client.partner = caller.id;
      });
  ...
  socket.on("call_reject", ({ targetUser, callee }) => {
    ...
    rooms[socket.room]?.find((client) => {
      if (client.id === targetUser) {
        (client.busy = false), (client.partner = null);
      }
    });
    rooms[socket.room]?.find((client) => {
      if (client.id === callee) {
  ...
  socket.on("call_ended", ({ target }) => {
    ...
    rooms[socket.room].forEach((client) => {
      if (client.id === target) {
        (client.partner = null), (client.busy = false);
      }
    });
  ...
  socket.on("disconnect", async () => {
    ...
    if (disconnectingUser.partner) {
      const partner = room.find(
        (client) => client.id === disconnectingUser.partner
      );
...
app.post("/logIn", async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: `Logging in as ${user.username}`, token, user });
  } catch (error) {
    // The safety net!
